// filepath: c:\Users\tim_a\Desktop\Commonplace book app\commonplace_book\lib\src\shared\notebook_constants.dart

class NotebookConstants {
  static const int maxNotebookNameLength = 100;
  static const int maxNotebookDescriptionLength = 500;
  
  // Structure constants
  static const int maxStructureDepth = 5;
  static const int maxFolderDepth = maxStructureDepth - 1;
  static const int maxPageDepth = maxStructureDepth;
  
  // Folder constants
  static const int maxFolderNameLength = 50;
  
  // Page constants
  static const int maxPageTitleLength = 50;
}